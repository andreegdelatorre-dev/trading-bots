...
def descargar_datos(symbol="ETHUSD#",
...
def backtesting(df, capital_inicial=10000, lote=0.01,
                stop_loss_usd=100, take_profit_usd=200):
    """
    ETH se mide en USD por unidad.
    Con lote 0.01 cada $1 de movimiento = $0.01

    stop_loss_usd    → cerrar si pierde $100
    take_profit_usd  → cerrar si gana  $200
    """
...
def graficar(historico, metricas):
    ...
    C_CAPITAL = "#627eea"    # color azul de Ethereum
    C_PRECIO  = "#555555"
    C_SELL    = "#ff4444"
    C_PANEL   = "#161616"
    C_GRID    = "#222222"
    ...
    ax1.plot(historico["fecha"], precio_norm,
             ...
             label="ETH precio (normalizado)", zorder=2)
    ...
    ax1.set_title(
        f"Backtesting ETH/USD (5min + BB + RSI)  |  "
    ...
    plt.savefig("backtest_eth_bb_rsi.png", dpi=150,
                bbox_inches="tight", facecolor="#0f0f0f")
    print("Grafica guardada como backtest_eth_bb_rsi.png")

# ── Ejecutar ──────────────────────────────────────────────
df = descargar_datos("ETHUSD#", mt5.TIMEFRAME_M5)
...
print("=" * 55)
print("   BACKTESTING ETH/USD (5min + BB + RSI)")
print("=" * 55)
