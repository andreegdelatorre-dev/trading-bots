...
def descargar_datos(symbol="BTCUSD#",
...
def generar_senales(df):
    """
    BUY  → precio toca banda inferior Y RSI < 35
    SELL → precio toca banda superior Y RSI > 65
    """
    df["senal"] = "HOLD"
...
def backtesting(df, capital_inicial=10000, lote=0.01,
                stop_loss_usd=500, take_profit_usd=1000):
    """
    BTC se mide en USD por unidad.
    Con lote 0.01 en BTC cada $1 de movimiento = $0.01

    stop_loss_usd    → cerrar si pierde $500
    take_profit_usd  → cerrar si gana $1000
    """
...
def graficar(historico, metricas):
    ...
    C_CAPITAL = "#f7931a"    # color naranja de Bitcoin
    C_PRECIO  = "#555555"
    C_SELL    = "#ff4444"
    C_PANEL   = "#161616"
    C_GRID    = "#222222"
    ...
    ax1.plot(historico["fecha"], precio_norm,
             ...
             label="BTC precio (normalizado)", zorder=2)
    ...
    ax1.set_title(
        f"Backtesting BTC/USD (5min + BB + RSI)  |  "
    ...
    plt.savefig("backtest_btc_bb_rsi.png", dpi=150,
                bbox_inches="tight", facecolor="#0f0f0f")
    print("Grafica guardada como backtest_btc_bb_rsi.png")

# ── Ejecutar ──────────────────────────────────────────────
df = descargar_datos("BTCUSD#", mt5.TIMEFRAME_M5)
...
print("=" * 55)
print("   BACKTESTING BTC/USD (5min + BB + RSI)")
print("=" * 55)
