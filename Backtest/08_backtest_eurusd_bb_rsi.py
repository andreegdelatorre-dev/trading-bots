...
def calcular_indicadores(df, bb_periodo=20, bb_desviacion=2,
                         rsi_periodo=14):
    """
    Misma estrategia que funcionó en el Oro:
    Bollinger Bands + RSI
    """
...
# ── Paso 4: Backtesting ───────────────────────────────────
def backtesting(df, capital_inicial=10000, comision=0.001,
                stop_loss_pips=7, take_profit_pips=21):
    """
    EUR/USD mide en pips igual que antes.
    Mantenemos SL 7 / TP 21 que fue el mejor resultado.
    """
    PIP         = 0.00001
    VALOR_PIP   = 0.10
...
def graficar(historico, metricas):
    ...
    ax1.plot(historico["fecha"], precio_norm,
             ...
             label="EUR/USD (normalizado)", zorder=2)
    ...
    ax1.set_title(
        f"Backtesting EUR/USD (5min + BB + RSI)  |  "
    ...
    plt.savefig("backtest_eurusd_bb_rsi.png", dpi=150,
                bbox_inches="tight", facecolor="#0f0f0f")
    print("Grafica guardada como backtest_eurusd_bb_rsi.png")
...
print("=" * 55)
print("   BACKTESTING EUR/USD (5min + BB + RSI)")
print("=" * 55)
