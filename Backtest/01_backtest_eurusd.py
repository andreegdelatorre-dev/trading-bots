...
def descargar_datos(symbol="EURUSD.sml",
    ...
    """
    Descarga las últimas 20,000 velas de 5 minutos.
    20,000 velas de 5min = ~69 días de datos
    """
    ...
    velas = mt5.copy_rates_from_pos(
        symbol, timeframe,
        0,        # desde la vela más reciente
        20000     # cantidad de velas hacia atrás
    )
...
def calcular_indicadores(df):
    """
    EMA9  = media exponencial rápida (45 minutos)
    EMA21 = media exponencial lenta  (105 minutos)
...
def generar_senales(df):
    """
    BUY  → EMA9 cruza hacia ARRIBA EMA21
            Y MACD histograma positivo

    SELL → EMA9 cruza hacia ABAJO EMA21
            Y MACD histograma negativo
    """
    df["senal"] = "HOLD"
...
def backtesting(df, capital_inicial=10000, lote=0.01,
    ...
    """
    capital_inicial  → $10,000 USD demo
    lote             → 0.01 = micro lote
    stop_loss_pips   → cerrar si pierde 10 pips
    take_profit_pips → cerrar si gana 15 pips

    En EUR/USD 1 pip = 0.0001
    Con micro lote 1 pip = $0.10
    """
    PIP         = 0.00001      # EURUSD.sml tiene 5 decimales
    VALOR_PIP   = 0.10
    ...
    for timestamp, fila in df.iterrows():
        ...
        if en_posicion:
            ...
            # Señal contraria cierra la posición
...
def graficar(historico, metricas):
    ...
    ax1.set_title(
        f"Backtesting EUR/USD (5min scalping)  |  "
    ...
    plt.savefig("backtest_eurusd_scalping.png", dpi=150,
                bbox_inches="tight", facecolor="#0f0f0f")
    print("Grafica guardada como backtest_eurusd_scalping.png")
...
print("=" * 55)
print("   BACKTESTING EUR/USD (5min scalping)")
print("=" * 55)
...
print(f"  Dias neutral         : {(retorno_diario == 0).sum()}")
